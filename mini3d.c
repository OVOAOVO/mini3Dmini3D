...
IUINT32 device_texture_read(const device_t* device, float u, float v) {
	...
	x1 = CMID(x - 1, 0, device->tex_width - 1);
	y1 = CMID(y - 1, 0, device->tex_height - 1);
	x2 = CMID(x + 1, 0, device->tex_width - 1);
	y2 = CMID(y + 1, 0, device->tex_height - 1);

	c1 = device->texture[y1][x1];
	c2 = device->texture[y1][x];
	c3 = device->texture[y][x1];
	c4 = device->texture[y][x];

	// 对四个相邻纹素进行二次线性差值计算
	IUINT32 r = (IUINT32)(((c1 >> 16) & 0xff) * (1 - uu) * (1 - vv) +
		((c2 >> 16) & 0xff) * uu * (1 - vv) +
		((c3 >> 16) & 0xff) * (1 - uu) * vv +
		((c4 >> 16) & 0xff) * uu * vv) & 0xff;

	IUINT32 g = (IUINT32)(((c1 >> 8) & 0xff) * (1 - uu) * (1 - vv) +
		((c2 >> 8) & 0xff) * uu * (1 - vv) +
		((c3 >> 8) & 0xff) * (1 - uu) * vv +
		((c4 >> 8) & 0xff) * uu * vv) & 0xff;

	IUINT32 b = (IUINT32)((c1 & 0xff) * (1 - uu) * (1 - vv) +
		(c2 & 0xff) * uu * (1 - vv) +
		(c3 & 0xff) * (1 - uu) * vv +
		(c4 & 0xff) * uu * vv) & 0xff;
...
void device_draw_primitive(device_t* device, const vertex_t* v1,
	...
	//背面剔除
	vector_t ti;
	matrix_apply(&ti, &device->transform,&device->transform.world); //world空间相机跟点坐标
	ti.z = ti.z -3.5;
	float cull  = vector_dotproduct(&ti,&normal);
	if (cull < 0) {
		return;
	}
